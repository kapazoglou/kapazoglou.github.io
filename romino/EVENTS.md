...
└── phase === 'rolled'  →  confirmTurn() then rollDice() (same click)
    ...
    │   ├── resolveSweepsAnimated() — beat (+ cube prelude when Dice & Cubes ON: suit cube fade, bottom die arc to roll btn) + sweep each run; releases held dice to pool (count only); bank stars × sum of sweep mults (longer runs → higher per-run mult); HUD equation → product → pip fly
    │   └── phase = 'idle'
