...
3. `commitScoringExit()` clears slots, fires next queued sweep (cross-line), calls `maybeOfferFourSquarePostSweepCard()` when done, then offers post-sweep cards or `checkPhaseTransition()`
