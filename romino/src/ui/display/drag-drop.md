...
v: 2.51
date: 2026-08-17
deps: [state, settings, row, dice-visual, placement-anim, render, placement-input, stack-swap-anim, sfx]
---
...
- **`snapping` ON + `directPlacement` ON** — semi-transparent `.placement-snap-ghost` at nearest valid slot during dice drag; `snap_tick` SFX when ghost appears or moves to a new slot; pointer flyer hidden while ghost has a slot (flyer only when no snap target); gap spread follows snap target; drop **re-resolves** snap at release coordinates then promotes ghost to commit flyer (never animates from pointer position); `state.snapGhostSlot` drives star-marker suppression beside the ghost; push-from-below excluded from snap listing (`PUSH_BELOW_DRAG_SNAP_ENABLED` — tap bottom die only)
