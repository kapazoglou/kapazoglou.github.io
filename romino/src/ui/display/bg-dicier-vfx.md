...
Decorative background texture: Dicier icon rows scroll vertically (generative/destructive ends swap on reversal) while the layer oscillates ±60–90° — ~3s ease ramps at each inflection, constant speed between; viewport `overflow: hidden` clips rows. Each grid cell independently cycles symbols on a random 12–45s timer with a subtle opacity fade (one swap at a time per cell). Entire layer speed scales with dice pool: full pool = baseline, empty pool = up to +25% faster (scroll, rotation, symbol cycles). Full template height; action bar overlay blends through.
