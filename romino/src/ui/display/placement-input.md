...
Red flash when: slot resolves but rules block it; `placeDieWithAnim` rejects; pointer on `#placement-row` but no slot (illegal zone) — unless `suppressInvalidFlash` (row reposition cancel). Monotonic-blocked stack completion flashes boundary anchor cube borders via `flashMonotonicBlocked`. No flash when dropping outside the row.
