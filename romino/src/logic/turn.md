...
v: 2.26
date: 2026-08-01
deps: [state, settings, dice, domino-roll, tile-deck, dealt-strip, deck-flank, deck-size, confirm-anim]
---
# Turn
...
`rollDice()` uses `drawDominoRoll()` when `dominoRoll` ON and `nRoll` is 2, 3, or 4; otherwise random spawn. Returns `'ok' | 'well-done' | null`. Cadence deal appends to `dealtStrip` when `tileDealtEvery` > 0 and `deckFlank` OFF.
