...
- **Dice Deck** — blankDie, blanksInRank, filterExtremes, sortDice, chooseDice
...
- `fillDiscovery` (default `true`) — requires `fourSquare`; discovery grid stacks cards bottom-up per rank column in conversion order (A…V headers); grid layout only.
- `fillDiscoveryEnd` (default `true`) — requires `fourSquare`; when ON, game ends when the fill-discovery bottom row is full or any two rank columns hold four cards.
...
- `allowFirstExtreme` (default `true`) — when OFF, forbids placing 1 or 6 as the first die on an empty card (any slot); with Scoring on, drag HUD coin onto a tray 1 or 6 to reroll to 2–5 (1 coin).
...
- `chooseDice` (default `false`) — when ON (incompatible with `diceDecks`), offers 6 dice in the action-bar tray with no upcoming preview and **no tray sort** (spawn/roll order preserved; `sortDice` inert); autoselects the **rightmost** selectable tray die; when the grid has a free card cell, after any 3 are placed the next card appears in-tray while the 3 unplaced dice stay reserved; **when the grid is full of cards**, all 6 dice remain placeable (no reserve); after all 6 are placed, a **Last Chance** card in the tray converts/scores — if no sweeps result, game over; otherwise capacity cards are offered until 6 adjacency slots are available; placing the card rolls 3 new dice merged with the reserved 3. Returning a placed die before card placement (below 3) removes the tray card and unlocks all dice.
